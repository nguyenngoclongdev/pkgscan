...
const gitResolver = require('@pnpm/git-resolver').default;
...
export function convertToLockfileFile(lockfile: Lockfile, opts: NormalizeLockfileOpts): LockfileFile {
    ...
    for (const [depPath, pkg] of Object.entries(lockfile.packages ?? {})) {
        snapshots[depPath] = pick(
            ['dependencies', 'optionalDependencies', 'transitivePeerDependencies', 'optional', 'id'],
            pkg
        );
...
function convertPkgIds(lockfile: LockfileFile): void {
    ...
    for (const [pkgId, pkg] of Object.entries(lockfile.packages ?? {})) {
        if (pkg.name) {
            ...
            if ('tarball' in pkg.resolution) {
                ...
                newId += gitResolver.createGitHostedPkgId(pkg.resolution);
    ...
    if (
        (lockfile.dependencies != null || lockfile.devDependencies != null || lockfile.optionalDependencies != null) &&
        !lockfile.importers?.['.']
    ) {
...
function convertProjectSnapshotToInlineSpecifiersFormat(
    ...
    const convertBlock = (block?: ResolvedDependencies) =>
        block != null ? convertResolvedDependenciesToInlineSpecifiersFormat(block, { specifiers }) : block;
    return {
        ...rest,
